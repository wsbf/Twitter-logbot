...
$(function() {
	...
	/**** signon: handles both sign on and change-dj ****/
	$("#dialog_signon").dialog({
		...
		buttons: {
			'Sign On': function() {
				...
				// On 'change dj,' doesn't sign out until next person signs in
				if(gotonewdj == 1 || automation == 1) {
	...
	// for actually signing off, not changing djs...i think
	$("#dialog_signoff").dialog( {
		...
		buttons: {
			'Sign Off': function() {
				...
				$.get("logging_logout.php", { id: showid, to_automation: true },
					function(data){
						//if (data != showid)
							//window.alert(data);
						showid = -1;
...
function refreshCurrShow() {
	...
	//		window.alert('poop');
			if(html == '-1') {
				...
				showid = row[0];
				// showid is from `show`, not scheduleid
				//window.alert('html not -1, showid is ' + showid);
				//window.alert('' + row[0] + ' ' + row[1] + ' ' + row[2] + ' ' + row[3] + ' ' + row[4] + ' ' + row[5] + ' ' + row[6]);
				starttime = row[1];
				$("#djname").text(row[2]);
				$("#showname").text(row[4]);
	...
	showTimer = setTimeout("refreshCurrShow()", 6000);
}
